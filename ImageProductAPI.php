...
class ImageProductAPI extends REST_Controller {
    public $dada;
    ...
    */
	...
	{
        try {
            $this->load->model('ImageProductModel');
            $this->load->helper("security"); 
            $stream = $this->security->xss_clean( $this->input->raw_input_stream );
            $data = json_decode(trim($stream), true);
            if(!empty($data["ID"])){
                $id = $data["ID"];
                $data = $this->ImageProductModel->getbyID($id);
            }else{
                $data = $this->ImageProductModel->getAll();
            }
